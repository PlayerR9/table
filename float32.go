...
// as it was an array of elements of type float32.
//
// Example:
//
//	[ a b c ]
...
func (t *Float32Table) Iterator() common.Iterater[float32] {
	iter := common.NewDynamicIterator(
		common.NewSimpleIterator(t.table),
		func(row []float32) common.Iterater[float32] {
...
func (t *Float32Table) Cleanup() {
	for i := 0; i < t.height; i++ {
		t.table[i] = make([]float32, t.width)
	}
}
...
func NewFloat32Table(width, height int) *Float32Table {
	...
	table := make([][]float32, height)
	for i := 0; i < height; i++ {
		table[i] = make([]float32, width)
	}
...
// coordinates return the zero value of type float32.
//
// Parameters:
//   - x: The x-coordinate of the cell.
//   - y: The y-coordinate of the cell.
