...
// NewByteTable creates a new table of type byte with the given width and height.
...
func NewByteTable(width, height int) (*ByteTable, error) {
	...
	table := make([][]byte, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]byte, width))
	}
...
// row as it was an array of elements of type byte.
//
// Example:
//
//	[ a b c ]
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t ByteTable) Row() iter.Seq[[]byte] {
	fn := func(yield func([]byte) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t ByteTable) CellAt(x, y int) byte {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type byte.
//
// Returns:
//   - [][]byte: The full table.
func (t ByteTable) FullTable() [][]byte {
	return t.table
}
...
func (t *ByteTable) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]byte, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *ByteTable) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]byte, new_height-t.height)...)
	}
