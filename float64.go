...
func NewFloat64Table(width, height int) (*Float64Table, error) {
	...
	table := make([][]float64, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]float64, width))
	}
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t Float64Table) Row() iter.Seq[[]float64] {
	fn := func(yield func([]float64) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t Float64Table) Height() int {
	return t.height
}
...
func (t Float64Table) CellAt(x, y int) float64 {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0.0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type float64.
//
// Returns:
//   - [][]float64: The full table.
func (t Float64Table) FullTable() [][]float64 {
	return t.table
}
...
func (t *Float64Table) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]float64, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *Float64Table) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]float64, new_height-t.height)...)
	}
