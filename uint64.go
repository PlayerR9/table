...
func NewUint64Table(width, height int) (*Uint64Table, error) {
	...
	table := make([][]uint64, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]uint64, width))
	}
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t Uint64Table) Row() iter.Seq[[]uint64] {
	fn := func(yield func([]uint64) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t Uint64Table) CellAt(x, y int) uint64 {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type uint64.
//
// Returns:
//   - [][]uint64: The full table.
func (t Uint64Table) FullTable() [][]uint64 {
	return t.table
}
...
func (t *Uint64Table) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]uint64, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *Uint64Table) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]uint64, new_height-t.height)...)
	}
