...
func NewIntTable(width, height int) (*IntTable, error) {
	...
	table := make([][]int, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]int, width))
	}
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t IntTable) Row() iter.Seq[[]int] {
	fn := func(yield func([]int) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t IntTable) CellAt(x, y int) int {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type int.
//
// Returns:
//   - [][]int: The full table.
func (t IntTable) FullTable() [][]int {
	return t.table
}
...
func (t *IntTable) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]int, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *IntTable) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]int, new_height-t.height)...)
	}
