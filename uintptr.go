...
// NewUintptrTable creates a new table of type uintptr with the given width and height.
...
func NewUintptrTable(width, height int) (*UintptrTable, error) {
	...
	table := make([][]uintptr, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]uintptr, width))
	}
...
// row as it was an array of elements of type uintptr.
//
// Example:
//
//	[ a b c ]
...
func (t UintptrTable) Cell() iter.Seq[uintptr] {
	fn := func(yield func(uintptr) bool) {
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t UintptrTable) Row() iter.Seq[[]uintptr] {
	fn := func(yield func([]uintptr) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t UintptrTable) Height() int {
	return t.height
}
...
func (t UintptrTable) CellAt(x, y int) uintptr {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type uintptr.
//
// Returns:
//   - [][]uintptr: The full table.
func (t UintptrTable) FullTable() [][]uintptr {
	return t.table
}
...
func (t *UintptrTable) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]uintptr, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *UintptrTable) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]uintptr, new_height-t.height)...)
	}
