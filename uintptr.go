...
// as it was an array of elements of type uintptr.
//
// Example:
//
//	[ a b c ]
...
func (t *UintptrTable) Iterator() common.Iterater[uintptr] {
	iter := common.NewDynamicIterator(
		common.NewSimpleIterator(t.table),
		func(row []uintptr) common.Iterater[uintptr] {
...
func (t *UintptrTable) Cleanup() {
	for i := 0; i < t.height; i++ {
		t.table[i] = make([]uintptr, t.width)
	}
}
...
func NewUintptrTable(width, height int) *UintptrTable {
	...
	table := make([][]uintptr, height)
	for i := 0; i < height; i++ {
		table[i] = make([]uintptr, width)
	}
...
// coordinates return the zero value of type uintptr.
//
// Parameters:
//   - x: The x-coordinate of the cell.
//   - y: The y-coordinate of the cell.
