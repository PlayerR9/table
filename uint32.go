...
// NewUint32Table creates a new table of type uint32 with the given width and height.
...
func NewUint32Table(width, height int) (*Uint32Table, error) {
	...
	table := make([][]uint32, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]uint32, width))
	}
...
// row as it was an array of elements of type uint32.
//
// Example:
//
//	[ a b c ]
...
func (t Uint32Table) Cell() iter.Seq[uint32] {
	fn := func(yield func(uint32) bool) {
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t Uint32Table) Row() iter.Seq[[]uint32] {
	fn := func(yield func([]uint32) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t Uint32Table) CellAt(x, y int) uint32 {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type uint32.
//
// Returns:
//   - [][]uint32: The full table.
func (t Uint32Table) FullTable() [][]uint32 {
	return t.table
}
...
func (t *Uint32Table) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]uint32, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *Uint32Table) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]uint32, new_height-t.height)...)
	}
