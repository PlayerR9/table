...
// NewInt8Table creates a new table of type int8 with the given width and height.
...
func NewInt8Table(width, height int) (*Int8Table, error) {
	...
	table := make([][]int8, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]int8, width))
	}
...
// row as it was an array of elements of type int8.
//
// Example:
//
//	[ a b c ]
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t Int8Table) Row() iter.Seq[[]int8] {
	fn := func(yield func([]int8) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t Int8Table) CellAt(x, y int) int8 {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type int8.
//
// Returns:
//   - [][]int8: The full table.
func (t Int8Table) FullTable() [][]int8 {
	return t.table
}
...
func (t *Int8Table) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]int8, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *Int8Table) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]int8, new_height-t.height)...)
	}
