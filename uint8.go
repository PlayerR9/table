...
func NewUint8Table(width, height int) (*Uint8Table, error) {
	...
	table := make([][]uint8, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]uint8, width))
	}
...
// row as it was an array of elements of type uint8.
//
// Example:
//
//	[ a b c ]
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t Uint8Table) Row() iter.Seq[[]uint8] {
	fn := func(yield func([]uint8) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t Uint8Table) CellAt(x, y int) uint8 {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type uint8.
//
// Returns:
//   - [][]uint8: The full table.
func (t Uint8Table) FullTable() [][]uint8 {
	return t.table
}
...
func (t *Uint8Table) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]uint8, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *Uint8Table) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]uint8, new_height-t.height)...)
	}
