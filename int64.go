...
// NewInt64Table creates a new table of type int64 with the given width and height.
...
func NewInt64Table(width, height int) (*Int64Table, error) {
	...
	table := make([][]int64, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]int64, width))
	}
...
// row as it was an array of elements of type int64.
//
// Example:
//
//	[ a b c ]
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t Int64Table) Row() iter.Seq[[]int64] {
	fn := func(yield func([]int64) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t Int64Table) CellAt(x, y int) int64 {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type int64.
//
// Returns:
//   - [][]int64: The full table.
func (t Int64Table) FullTable() [][]int64 {
	return t.table
}
...
func (t *Int64Table) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]int64, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *Int64Table) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]int64, new_height-t.height)...)
	}
