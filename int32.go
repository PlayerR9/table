...
// NewInt32Table creates a new table of type int32 with the given width and height.
...
func NewInt32Table(width, height int) (*Int32Table, error) {
	...
	table := make([][]int32, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]int32, width))
	}
...
// row as it was an array of elements of type int32.
//
// Example:
//
//	[ a b c ]
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t Int32Table) Row() iter.Seq[[]int32] {
	fn := func(yield func([]int32) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t Int32Table) CellAt(x, y int) int32 {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type int32.
//
// Returns:
//   - [][]int32: The full table.
func (t Int32Table) FullTable() [][]int32 {
	return t.table
}
...
func (t *Int32Table) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]int32, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *Int32Table) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]int32, new_height-t.height)...)
	}
