...
// NewStringTable creates a new table of type string with the given width and height.
...
func NewStringTable(width, height int) (*StringTable, error) {
	...
	table := make([][]string, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]string, width))
	}
...
// row as it was an array of elements of type string.
//
// Example:
//
//	[ a b c ]
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t StringTable) Row() iter.Seq[[]string] {
	fn := func(yield func([]string) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t StringTable) CellAt(x, y int) string {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return ""
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type string.
//
// Returns:
//   - [][]string: The full table.
func (t StringTable) FullTable() [][]string {
	return t.table
}
...
func (t *StringTable) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]string, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *StringTable) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]string, new_height-t.height)...)
	}
