...
// NewComplex64Table creates a new table of type complex64 with the given width and height.
...
func NewComplex64Table(width, height int) (*Complex64Table, error) {
	...
	table := make([][]complex64, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]complex64, width))
	}

	return &Complex64Table{
		table:  table,
		width:  width,
		height: height,
	}, nil
...
// row as it was an array of elements of type complex64.
//
// Example:
//
//	[ a b c ]
...
func (t Complex64Table) Cell() iter.Seq[complex64] {
	fn := func(yield func(complex64) bool) {
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t Complex64Table) Row() iter.Seq[[]complex64] {
	fn := func(yield func([]complex64) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t Complex64Table) Width() int {
	return t.width
}
...
func (t Complex64Table) Height() int {
	return t.height
}
...
func (t Complex64Table) CellAt(x, y int) complex64 {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type complex64.
//
// Returns:
//   - [][]complex64: The full table.
func (t Complex64Table) FullTable() [][]complex64 {
	return t.table
}
...
func (t *Complex64Table) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]complex64, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *Complex64Table) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]complex64, new_height-t.height)...)
	}
