...
func NewComplex128Table(width, height int) (*Complex128Table, error) {
	...
	table := make([][]complex128, 0, height)
	for i := 0; i < height; i++ {
		table = append(table, make([]complex128, width))
	}

	return &Complex128Table{
		table:  table,
		width:  width,
		height: height,
	}, nil
...
//	Row(0) -> [ a b c ]
//	Row(1) -> [ d e f ]
func (t Complex128Table) Row() iter.Seq[[]complex128] {
	fn := func(yield func([]complex128) bool) {
		for i := 0; i < t.height; i++ {
			if !yield(t.table[i]) {
				return
...
func (t Complex128Table) Width() int {
	return t.width
}
...
func (t Complex128Table) Height() int {
	return t.height
}
...
func (t Complex128Table) CellAt(x, y int) complex128 {
	if x < 0 || x >= t.width || y < 0 || y >= t.height {
		return 0
	} else {
		return t.table[y][x]
...
// FullTable returns the full table as a 2D slice of elements of type complex128.
//
// Returns:
//   - [][]complex128: The full table.
func (t Complex128Table) FullTable() [][]complex128 {
	return t.table
}
...
func (t *Complex128Table) ResizeWidth(new_width int) error {
	if t == nil {
	...
	if new_width == t.width {
		...
		for i := 0; i < t.height; i++ {
			t.table[i] = append(t.table[i], make([]complex128, new_width-t.width)...)
		}
	}
...
//   - errors.NilReceiver: If the table is nil.
func (t *Complex128Table) ResizeHeight(new_height int) error {
	if t == nil {
		return errors.NilReceiver
	...
	if new_height == t.height {
		...
		t.table = append(t.table, make([][]complex128, new_height-t.height)...)
	}
